...
class IssueViewColumnsController < ApplicationController
  ...
  before_action :build_query_for_project, only: [:update]
  before_action :check_user_logged_in_and_has_rights, only: [:update_collapsed_ids]
  ...
  def check_user_logged_in_and_has_rights
    @issue = Issue.find(params[:id])
